...
def write_file_preview(working_directory, file_path, content, run_id, log_changes=True):
    ...
    try:
        ...
        if os.path.exists(full_path):
            save_file(run_id, source_path=full_path, content=content, log_changes=log_changes, backup=backup)
            return f'Save proposed changes to "{file_path}" in __ai_outputs__ ({len(content)} characters to be written)'
        
        else:
            file_name = os.path.basename(full_path)
            save_file(run_id, file_name=file_name, content=content, log_changes=log_changes, backup=backup)
